...
def count_repeats(xs, x):
    ...
    def secondfunc(left2, right2):
        ...
        if xs[mid2] == x:
            left2 = mid2 + 1
        return secondfunc(left2, right2)

    if len(xs) == 0:
        return 0

    big = secondfunc(0, len(xs) - 1)
    small = firstfunc(0, len(xs) - 1)
...
def argmin(f, lo, hi, epsilon=1e-3):
    ...
    The basic algorithm is:
        ...
        2) For each recursive call:
            ...
            b) one of the 4 points (lo,m1,m2,hi) must be the smallest;
               ...
               you recursively call your function on the interval [lo
               ,m2] or [m1,hi]
    ...
    we will spend a lot of time talking about different f funct
    ions that can be minimized and their applications.
    ...
    should look like.
    Your output numbers are likely to be slightly different due to
